...
import { preloaderContent } from "../../data/preloader";
...
export function Preloader({ onComplete, onRevealStart }: PreloaderProps) {
  ...
  const logs = preloaderContent.logs;
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 w-full h-full bg-midnight z-[100] flex flex-col justify-between p-8 md:p-12 select-none overflow-hidden"
      style={{ fontFamily: 'var(--font-mono)' }}
    >
      {/* Top Header */}
      <div className="flex items-center justify-between text-[10px] text-slate tracking-[0.25em] uppercase">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-violet animate-ping" />
          <span>{preloaderContent.systemBooting}</span>
        </div>
        <span>{preloaderContent.nodeText}</span>
      </div>

      {/* Centered Percentage */}
      <div className="flex-1 flex flex-col justify-center items-center relative py-12">
        {/* Glow blobs in background */}
        <div className="absolute w-80 h-80 rounded-full bg-violet/5 blur-[120px] pointer-events-none" />
        <div className="absolute w-60 h-60 rounded-full bg-cosmic-a/5 blur-[100px] pointer-events-none" />

        <div ref={progressTextRef} className="text-center relative z-10">
          <div
            className="text-[clamp(4.5rem,14vw,11rem)] text-whisper font-light leading-none tracking-tighter cursor-default"
            style={{
          ...
          >
            ...
            <span className="text-violet font-medium text-[clamp(1.5rem,5vw,3rem)] ml-1">
              %
            </span>
          </div>
          <div className="h-1.5 w-64 md:w-80 bg-white/5 rounded-full mt-6 overflow-hidden mx-auto border border-white/[0.04]">
            <div
              className="h-full bg-gradient-to-r from-cosmic-a to-violet rounded-full shadow-[0_0_15px_rgba(175,80,255,0.7)] transition-all duration-75 ease-out"
        ...
        {logs.slice(0, logIndex + 1).map((log, index) => (
          <div
            ...
            className={`preloader-log text-[11px] font-medium tracking-wide flex items-center gap-2 ${
              index === logs.length - 1 ? "text-violet" : "text-slate"
            }`}
          >
