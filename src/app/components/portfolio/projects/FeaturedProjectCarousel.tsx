...
export function FeaturedProjectCarousel({ projects }: FeaturedProjectCarouselProps) {
  ...
  return (
    ...
    >
      <div className="featured-project-glow pointer-events-none absolute left-[8%] top-[12%] h-[380px] w-[380px] rounded-full bg-violet/12 blur-[100px]" />
      <div className="pointer-events-none absolute bottom-[5%] right-[-10%] h-[340px] w-[340px] rounded-full bg-cosmic-a/10 blur-[96px]" />

      <div className="relative z-10 mx-auto grid w-full max-w-[1440px] gap-20 px-5 md:gap-28 md:px-8">
        {projects.map((project, index) => (
          <div
            key={project.title}
            className="featured-project-row grid min-h-[calc(100dvh-8rem)] items-center gap-10 [transform:translateZ(0)] md:grid-cols-[minmax(0,0.9fr)_minmax(420px,1.1fr)] md:gap-12 md:[contain:paint]"
          >
